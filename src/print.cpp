...
// Using atomic for potential multi-threading from the host side launching kernels.
// If only one host thread launches kernels concurrently, simple types might suffice.
static std::atomic<unsigned long long> g_currentKernelTotalBytes = {0};
static std::atomic<bool> g_timingStarted = {false};
static std::atomic<bool> g_timingEnded = {false};
...
extern "C" {
	extern __constant__ int d_targetStartLine;
	extern __constant__ int d_targetEndLine;
	...
	void reportMemoryBandwidth(void);
...
// Called by the instrumented host code after measureKernel(2)
void reportMemoryBandwidth() {
	if (g_timingStarted && g_timingEnded) {
    
        std::chrono::duration<double> duration = totalElapsedTime;
        double duration_sec = duration.count();

        unsigned long long totalBytes = g_currentKernelTotalBytes.load();

        if (duration_sec > 1e-9) { // Use a small threshold
            double bandwidth_bytes_per_sec = static_cast<double>(totalBytes) / duration_sec;
            // Convert Bytes/sec to GB/s 
            double bandwidth_gb_per_sec = bandwidth_bytes_per_sec / (1024.0 * 1024.0 * 1024.0);

            printf("[CUDAAdvisor BW] Kernel Execution Time: %.6f ms\n", duration_sec * 1000.0);
            printf("[CUDAAdvisor BW] Total Bytes Transferred (Kernel): %llu bytes\n", totalBytes);
            printf("[CUDAAdvisor BW] Achieved Memory Bandwidth: %.3f GB/s\n", bandwidth_gb_per_sec);

        } else {
            printf("[CUDAAdvisor BW] Kernel Execution Time too short (%.9f s) to calculate bandwidth reliably.\n", duration_sec);
            printf("[CUDAAdvisor BW] Total Bytes Transferred (Kernel): %llu bytes\n", totalBytes);
        }

        g_timingStarted = false;
        g_timingEnded = false;

    } else {
         printf("[CUDAAdvisor BW] Error: reportMemoryBandwidth called without valid start/end timing data.\n");
    }
}
...
void setLineRangeTarget(int startLine, int endLine) {
    g_targetStartLine = startLine;
    g_targetEndLine = endLine;

	int deviceStartLine = (startLine > 0) ? startLine : -1;
    int deviceEndLine = (endLine > 0 && endLine >= deviceStartLine) ? endLine : -1;

    // If start is positive but end isn't, default end to start (measure single line)
    // Or adjust logic as needed - current setup requires both > 0.
    // Let's stick to requiring both > 0 for a valid range.
    if (deviceStartLine <= 0 || deviceEndLine <= 0) {
        deviceStartLine = -1;
        deviceEndLine = -1;
    }


    cudaError_t errStart = cudaMemcpyToSymbol(d_targetStartLine, &deviceStartLine, sizeof(int), 0, cudaMemcpyHostToDevice);
    cudaError_t errEnd = cudaMemcpyToSymbol(d_targetEndLine, &deviceEndLine, sizeof(int), 0, cudaMemcpyHostToDevice);

    if (errStart != cudaSuccess || errEnd != cudaSuccess) {
        fprintf(stderr, "[CUDAAdvisor BW] Error copying line range to device constant memory: %s / %s\n",
                cudaGetErrorString(errStart), cudaGetErrorString(errEnd));
        // Handle error appropriately, maybe revert host state?
        g_targetStartLine = -1;
        g_targetEndLine = -1;
    }
    ...
    if (deviceStartLine > 0 && deviceEndLine > 0) {
         printf("[CUDAAdvisor BW] Set target line range (Host & Device): %d - %d\n", deviceStartLine, deviceEndLine);
    } else {
         printf("[CUDAAdvisor BW] Reset target line range (Host & Device) (measuring whole kernel).\n");
         // Ensure host range is also reset if device copy failed or input was invalid
         g_targetStartLine = -1;
         g_targetEndLine = -1;
    }
}
